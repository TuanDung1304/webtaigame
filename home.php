...
if($search != ''){
    ...
    $apps = executeResult('select * from application where title like "%'
    .$search.'%" limit '.$item_per_page.' offset '.$offset.'');

    $total_records = mysqli_query($conn, 'select * from application where title like "%'.$search.'%"')->num_rows;
}
